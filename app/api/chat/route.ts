...
export async function POST(request: NextRequest) {
  try {
    ...
    const countryContext = selectedCountry
      ? `Respond using local terminology and expressions appropriate for ${selectedCountry}. `
      : "";
...
- Use local terminology and expressions when appropriate for ${
      selectedCountry || "the user's region"
    }
